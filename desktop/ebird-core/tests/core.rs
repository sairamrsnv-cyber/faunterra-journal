...
fn sanitize_removes_identity_and_reports_only_what_was_there() {
    ...
    assert!(
        back.contains("OBS1") && back.contains("S1"),
...
fn archive_allows_an_ordinary_path_outside_any_mount_root() {
    ...
    assert!(
        !loc.guarded,
        "an ordinary directory is not a guarded volume"
    );
}
...
fn writes_are_atomic_and_leave_no_temp_file_behind() {
    ...
    assert!(
        !dir.join("IN/2026-01-02.json.tmp").exists(),
    ...
    assert_eq!(
        idx,
        vec![("IN".to_string(), vec!["2026-01-02".to_string()])]
    );
}
...
fn token_is_masked_down_to_its_last_four() {
    ...
    assert!(
        !m.contains("abcdef"),
        "no part of the key beyond the tail may appear"
    );
    assert_eq!(
        EbirdClient::new("abc", None).unwrap().masked_token(),
        "****"
    );
}
...
fn live_is_true_only_for_the_real_endpoint() {
    ...
    assert!(EbirdClient::new("k", Some(ebird_core::DEFAULT_BASE.into()))
        .unwrap()
        .is_live());
    assert!(
        !EbirdClient::new("k", Some("http://127.0.0.1:8788/v2".into()))
            .unwrap()
            .is_live()
    );
}
...
fn request_url_carries_the_parameters_the_archive_depends_on() {
    ...
    assert!(
        u.contains("detail=full"),
        "without full detail there is no location or count"
    );
...
async fn full_pull_writes_clean_days_and_second_run_spends_nothing() {
    ...
    assert!(first.rows > 0);
    assert!(
        kinds.contains(&"privacy".to_string()),
        "a strip must be reported"
    );
    ...
    for (region, days) in archive::index(&dir).unwrap() {
        for date in days {
            ...
            assert!(
                !raw.contains("userDisplayName"),
                "{region}/{date} leaked a contributor name"
            );
            let parsed: ebird_core::DayFile = serde_json::from_str(&raw).unwrap();
            assert!(
                !parsed.live,
                "fixture rows must be marked synthetic in the file itself"
            );
    ...
    assert_eq!(
        second.skipped, 2,
        "an archived day must never be re-fetched"
    );
...
async fn a_rejected_key_fails_loudly_and_writes_nothing() {
    ...
    assert!(
        reasons[0].contains("403") && reasons[0].contains("EBIRD_API_TOKEN"),
        "the error must name the fix, not just the status: {}",
        reasons[0]
    );
    assert!(
        archive::index(&dir).unwrap().is_empty(),
...
async fn summary_counts_all_species_not_just_the_top_slice() {
    ...
    assert!(
        s.species > 3,
        "distinct species ({}) must exceed the top slice",
        s.species
    );
    ...
    assert!(
        s.caveat.contains("effort"),
        "every summary carries the effort caveat"
    );
